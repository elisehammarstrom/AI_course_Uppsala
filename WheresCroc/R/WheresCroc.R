...
getTransitionModel <- function(edges) {
  ...
  for (i in 1:num_waterholes) {
    options <- getOptions(i, edges) #options is which nodes are adjacent to current node
    #print(options)
    prob <- 1 / length(options)  # As we at node i can have m adjacent nodes connecting to it
    for (j in options) {
...
# calculate how likely it is to get a specific reading if we assume the croc is in that specific waterhole
# high dnorm values say high relative likelihood
getEmissionProb <- function(readings, probs, waterhole_number) {
  ...
  pho_prob <- dnorm(readings[2], mean=probs$phosphate[waterhole_number,1], sd=probs$phosphate[waterhole_number,2])
  nit_prob <- dnorm(readings[3], mean=probs$nitrogen[waterhole_number,1], sd=probs$nitrogen[waterhole_number,2])
  return(sal_prob * pho_prob * nit_prob)
}

predictCrocMovementOneStep <- function(current_probs, transition_matrix) {
  predicted_probs <- transition_matrix %*% current_probs
  return(predicted_probs)
}

runForwardAlgo <- function(current_probs, transition_matrix, readings, probs) {
  predicted_probs = predictCrocMovementOneStep(current_probs, transition_matrix)

  emission_probs <- sapply(1:40, function(i) getEmissionProb(readings, probs, i))
  updated_probs <- predicted_probs * emission_probs 

  updated_probs <- updated_probs / sum(updated_probs)
...
myWC=function(moveInfo,readings,positions,edges,probs) {
  ...
  options=getOptions(current_pos,edges)
  
  # initialize first load
  if (is.null(moveInfo$mem$transition_matrix)) {
    moveInfo$mem$transition_matrix <- getTransitionModel(edges)
    moveInfo$mem$current_probs <- rep(1/40, 40)  # Initial uniform distribution at time t0
  }
  ...
  moveInfo$mem$current_probs <- updated_probs
